...
function App() {
  ...
  function addToken(token) {
    setTokens((prevTokens) => [
      ...prevTokens,
      { name: token.name, address: token.address, contractAddress:token.contractAddress },
    ]);
  }
  ...
  }, 12*3600*1000); //Call every 12 hours
      ...
      {tokens.map((token) => {
        return (
          <div className="m-10 bg-gray-100 p-4 rounded-md shadow-md mb-4">
            <div className="text-xl font-semibold mb-2">Token Name: {token.name}</div>
            <div className="font-bold overflow-scroll">Token Address:{token.address}</div>
            <div className="font-bold overflow-scroll">Contract Address: {token.contractAddress}</div>
            <div className="font-bold">Token Balance: {token.balance[token.balance.length-1]}</div>
            <br/>
          </div>
        )
      })}
